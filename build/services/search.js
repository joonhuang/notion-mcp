 ...
 */
export function registerToolsForSearch(server) {
    // specificallt for pages
    server.tool("search-notion-page", { keyword: z.string() }, async ({ keyword }) => {
        const request = {
            query: keyword,
            filter: {
                value: "page",
    ...
    server.tool("search-notion-database", { keyword: z.string() }, async ({ keyword }) => {
        const request = {
            query: keyword,
            filter: {
                value: "database",
