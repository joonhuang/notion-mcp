...
export function registerToolsForPage(server) {
    server.tool("create-notion-page", { pageData: z.object({
            parent: z.object({
    ...
    server.tool("get-notion-page", { pageId: z.string() }, async ({ pageId }) => {
        const data = await getPage(pageId);
        return { content: [{ type: "text", text: JSON.stringify(data) }] };
    });
    server.tool("update-notion-page", { pageId: z.string(), properties: z.record(z.string(), z.any()) }, async ({ pageId, properties }) => {
        // Validate that properties have valid Notion API structure
        const formattedProperties = Object.entries(properties).reduce((acc, [key, value]) => {
    ...
    server.tool("delete-notion-page", { pageId: z.string() }, async ({ pageId }) => {
